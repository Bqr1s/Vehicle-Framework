...
namespace Vehicles
{
  public static class AltitudeMeter
  {
    ...
    public static Vector2 AltitudeScreenPos =>
      new Vector2(0, PaneTopY - (WindowHeight + InfoWindoHeight));
    ...
    public static void DrawAltitudeMeter(AerialVehicleInFlight aerialVehicle)
    {
      try
      {
        ...
        float elevation =
          (MeterSize.y - (aerialVehicle.Elevation / MaximumAltitude * MeterSize.y)).Clamp(
            MaxAltitudeScreenHeight, MeterSize.y - MinAltitudeScreenHeight);
        Find.WindowStack.ImmediateWindow(aerialVehicle.GetHashCode(), windowRect,
          WindowLayer.GameUI, delegate()
          {
            ...
            GUI.BeginScrollView(windowRect, new Vector2(windowRect.x, elevation - WindowHeight / 2),
              viewRect, GUIStyle.none, GUIStyle.none);
            ...
            if (elevation <= MaximumAltitude)
            {
              ...
              GUI.DrawTexture(lineRect,
                elevation >= MeterSize.y / 2 ? BaseContent.BlackTex : BaseContent.WhiteTex);
            }

            GUI.color = WindowBGBorderColor;
            Widgets.DrawLineHorizontal(0, windowRect.y + elevation + MeterSize.y / 2,
              viewRect.width);
            ...
            Rect labelRect = new Rect(viewRect.width + 5, windowRect.y + elevation - textHeight / 2,
              viewRect.width - 5, textHeight);
      ...
      {
        SmashLog.Error(
          $"Exception thrown while trying to draw <type>AltitudeMeter</type> for {aerialVehicle?.Label ?? "NULL"}. Exception=\"{ex}\"");
      }
    }
  }
}