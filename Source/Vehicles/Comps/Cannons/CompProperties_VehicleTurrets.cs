...
namespace Vehicles
{
  ...
  public class CompProperties_VehicleTurrets : VehicleCompProperties
  {
    ...
    [ActionOnSettingsInput(typeof(CompProperties_VehicleTurrets), nameof(RecacheAllTurrets))]
      ...
      if (!string.IsNullOrEmpty(turret.parentKey))
      {
        foreach (VehicleTurret parentTurret in turrets.Where(c => c.key == turret.parentKey))
        {
          if (turret.parentKey == parentTurret.key)
          {
            ...
            {
              Log.Error(
                $"Recursive turret attachments detected, this is not allowed. Disconnecting turret from parent.");
    ...
    public override IEnumerable<string> ConfigErrors(ThingDef parentDef)
    {
      ...
      {
        yield return
          "<field>parentDef</field> must be a <type>VehicleDef</type> in order to implement <type>CompVehicleTurrets</type>."
           .ConvertRichText();
      }
    }
