...
namespace Vehicles
{
  ...
  public class CompVehicleLauncher : VehicleComp
  {
    ...
    public virtual float ClimbRateStat
    {
      get
      {
        bool flight = launchProtocol.CanLaunchNow && FlightSpeed > 0 &&
          (!Vehicle.CompFueledTravel?.EmptyTank ?? true);
        if (!flight)
        {
    ...
    public IEnumerable<VehicleTurret> StrafeTurrets
    {
      get
      {
        ...
        {
          Log.Error(
            $"Cannot retrieve <property>StrafeTurrets</property> with no <type>CompCannons</type> comp.");
          yield break;
        }
    ...
    public ShuttleLaunchStatus GetShuttleStatus(GlobalTargetInfo mouseTarget, Vector3 origin)
    {
      ...
      else if (Vehicle.CompFueledTravel != null)
      {
        ...
        else if (FuelNeededToLaunchAtDist(origin, mouseTarget.Tile) >
          (Vehicle.CompFueledTravel.Fuel - LaunchTargeter.TotalFuelCost))
        {
    ...
    public override string CompInspectStringExtra()
    {
      ...
      {
        return "NotReadyForLaunch".Translate() + ": " +
          "TransportPodInGroupHasSomethingLeftToLoad".Translate().CapitalizeFirst() + ".";
      }
      ...
      bool recon = false)
    {
      ...
      GenSpawn.Spawn(vehicleLeaving, Vehicle.Position, Vehicle.Map,
        ...
        Vehicle.Rotation, WipeMode.Vanish);
    ...
    public void InitLaunchProtocol()
    {
      ...
      launchProtocol ??= (LaunchProtocol)Activator.CreateInstance(Props.launchProtocol.GetType(),
        [Props.launchProtocol, Vehicle]);
    }
