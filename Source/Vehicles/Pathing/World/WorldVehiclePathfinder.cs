...
namespace Vehicles
{
  ...
  public class WorldVehiclePathfinder : WorldComponent
  {
    ...
    private readonly TileFeatureLookup tileFeatureLookup;
    ...
    /// </summary>
    public WorldPath FindPath(int startTile, int destTile, VehicleCaravan caravan,
      Func<float, bool> terminator = null)
    {
      return FindPath(startTile, destTile, caravan.VehiclesListForReading, caravan.TicksPerMove,
        terminator);
    }
    ...
    /// </summary>
    public WorldPath FindPath(int startTile, int destTile, List<VehiclePawn> vehicles,
      int ticksPerMove = VehicleCaravanTicksPerMoveUtility.DefaultTicksPerMove,
      Func<float, bool> terminator = null)
    {
      return FindPath(startTile, destTile, vehicles.UniqueVehicleDefsInList(), ticksPerMove,
        terminator);
    }
      ...
      Func<float, bool> terminator = null)
    {
      ...
      try
      {
        ...
        {
          Log.Error(
            $"Tried to FindPath with invalid startTile={startTile} vehicles={vehiclesPathing}");
          return WorldPath.NotFound;
        }
        if (destTile < 0)
        {
          Log.Error(
            $"Tried to FindPath with invalid destTile={destTile} vehicles={vehiclesPathing}");
          return WorldPath.NotFound;
        }
        if (!vehicleDefs.All(vehicleDef =>
          WorldVehiclePathGrid.Instance.reachability.CanReach(vehicleDef, startTile, destTile)))
        {
          return WorldPath.NotFound;
        }
                ...
                if (calcGrid[neighbor].status != statusClosedValue)
                {
                  ...
                  )
                  {
                    ...
                    if (coastalTravel)
                    {
                      ...
                      {
                        highestTerrainCost = vehicleDefs.Max(vehicleDef =>
                    ...
                    bool diffStatusValues =
                      status != statusClosedValue && status != statusOpenValue;
                    if (diffStatusValues || calcGrid[neighbor].knownCost > totalPathCost)
                    {
                      Vector3 tileCenter = grid.GetTileCenter(neighbor);
                      if (diffStatusValues)
                      {
                        float tileDistance = grid.ApproxDistanceInTiles(
                          GenMath.SphericalDistance(tileCenter.normalized, normalized));
                        calcGrid[neighbor].heuristicCost = Mathf.RoundToInt(ticksPerMove *
                          tileDistance * heuristicStrength * bestRoadDiscount);
                      }
                      ...
                      if (DebugHelper.World.VehicleDef != null)
                      {
                        ...
                        {
                          Find.World.debugDrawer.FlashTile(neighbor,
                            colorPct: calcGrid[neighbor].knownCost / 150f,
        ...
        Log.Warning(
          $"{vehiclesPathing} pathing from {startTile} to {destTile} ran out of tiles to process.");
        return WorldPath.NotFound;
      }
      finally
      {
