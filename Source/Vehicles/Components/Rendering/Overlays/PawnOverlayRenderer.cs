...
{
  private Listing_SplitColumns listing = new();
  ...
  public Vector3 DrawOffsetFor(Rot8 rot)
  {
    Vector3 offset = rot.AsInt switch
    {
      ...
      1 => drawOffsetEast ??
        drawOffsetWest?.MirrorHorizontal() ?? drawOffset.RotatedBy(rot.AsAngle),
      2 => drawOffsetSouth ??
        drawOffsetNorth?.MirrorVertical() ?? drawOffset.RotatedBy(rot.AsAngle),
      3 => drawOffsetWest ??
        drawOffsetEast?.MirrorHorizontal() ?? drawOffset.RotatedBy(rot.AsAngle),
      4 => drawOffsetNorthEast ?? drawOffsetNorthWest?.MirrorHorizontal() ??
        drawOffsetNorth?.RotatedBy(45) ?? drawOffset.RotatedBy(rot.AsAngle),
      5 => drawOffsetSouthEast ?? drawOffsetSouthWest?.MirrorHorizontal() ??
        drawOffsetSouth?.RotatedBy(-45) ?? drawOffset.RotatedBy(rot.AsAngle),
      6 => drawOffsetSouthWest ?? drawOffsetSouthEast?.MirrorHorizontal() ??
        drawOffsetSouth?.RotatedBy(45) ?? drawOffset.RotatedBy(rot.AsAngle),
      7 => drawOffsetNorthWest ?? drawOffsetNorthEast?.MirrorHorizontal() ??
        drawOffsetNorth?.RotatedBy(-45) ?? drawOffset.RotatedBy(rot.AsAngle),
      _ => throw new NotImplementedException(),
    };
  ...
  public void RenderEditor(Rect rect)
  {
    ...
    if (layerNorth != null)
    {
      ...
      listing.SliderLabeled("Layer North", ref value, string.Empty, string.Empty, string.Empty, -5,
        5);
      layerNorth = value;
    }
    if (layerEast != null)
    {
      ...
      listing.SliderLabeled("Layer East", ref value, string.Empty, string.Empty, string.Empty, -5,
        5);
      layerEast = value;
    }
    if (layerSouth != null)
    {
      ...
      listing.SliderLabeled("Layer South", ref value, string.Empty, string.Empty, string.Empty, -5,
        5);
      layerSouth = value;
    }
    if (layerWest != null)
    {
      ...
      listing.SliderLabeled("Layer West", ref value, string.Empty, string.Empty, string.Empty, -5,
        5);
      layerWest = value;
    }
    listing.NextRow();
