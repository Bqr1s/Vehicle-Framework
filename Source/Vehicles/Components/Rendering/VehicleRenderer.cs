...
namespace Vehicles
{
  public sealed class VehicleRenderer
  {
    ...
    public PawnFirefoamDrawer FirefoamOverlays =>
      throw new NotImplementedException(); // firefoamOverlays;
    ...
    [Obsolete("Use new transform based rendering instead.", error: true)]
    public void RenderPawnAt_TEMP(Vector3 drawLoc, Rot8 rot, float extraRotation,
      bool northSouthRotation)
    {
      if (vehicle.Spawned)
      {
      ...
      TransformData transform = new(drawLoc, rot, extraRotation);
      RenderVehicleWithOverlays(in transform);
    }

    // Vehicle body rendering + overlays
    [Obsolete("Use new transform based rendering instead.", error: true)]
    private void RenderVehicle(Vector3 rootLoc, Rot8 rot, float extraRotation,
      bool northSouthRotation)
    {
      if (vehicle.Spawned)
      {
