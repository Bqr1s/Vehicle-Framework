...
namespace Vehicles
{
  public class Dialog_StashVehicle : Window
  {
    ...
    private float SourceMassUsage
    {
      get
      {
        if (sourceMassUsageDirty)
        {
          ...
          cachedSourceMassUsage = CollectionsMassCalculator.MassUsageTransferables(transferables,
            IgnorePawnsInventoryMode.Ignore, false, false);
        }
    ...
    private float SourceMassCapacity
    {
      get
      {
        if (sourceMassCapacityDirty)
        {
          ...
          cachedSourceMassCapacity =
            capacity; //CollectionsMassCalculator.CapacityTransferables(this.transferables, stringBuilder);
    ...
    private float SourceTilesPerDay
    {
      get
      {
        if (sourceTilesPerDayDirty)
        {
          ...
          cachedSourceTilesPerDay = TilesPerDayCalculator.ApproxTilesPerDay(
            CaravanTicksPerMoveUtility.GetTicksPerMove(PawnsEmbarking.ToList(), SourceMassUsage,
              SourceMassCapacity, explanation: explanation), caravan.Tile, -1,
          ...
          cachedSourceTilesPerDayExplanation = explanation.ToString();
        }
    ...
    private Pair<ThingDef, float> SourceForagedFoodPerDay
    {
      get
      {
        if (sourceForagedFoodPerDayDirty)
        {
          ...
          StringBuilder stringBuilder = new StringBuilder();
          cachedSourceForagedFoodPerDay = ForagedFoodPerDayCalculator.ForagedFoodPerDay(
            PawnsEmbarking.ToList(), Biome, Faction.OfPlayer, true, false, stringBuilder);
    ...
    private float SourceVisibility
    {
      get
      {
        if (sourceVisibilityDirty)
        {
          ...
          StringBuilder stringBuilder = new StringBuilder();
          cachedSourceVisibility =
            CaravanVisibilityCalculator.Visibility(PawnsEmbarking, true, stringBuilder);
    ...
    private float DestMassUsage
    {
      get
      {
        if (destMassUsageDirty)
        {
          ...
          cachedDestMassUsage = CollectionsMassCalculator.MassUsageTransferables(transferables,
            IgnorePawnsInventoryMode.Ignore, false, false);
        }
    ...
    private float DestMassCapacity
    {
      get
      {
        if (destMassCapacityDirty)
        {
          ...
          StringBuilder explanation = new();
          cachedDestMassCapacity =
            CollectionsMassCalculator.CapacityTransferables(transferables, explanation);
          cachedDestMassCapacityExplanation = explanation.ToString();
        }
    ...
    private float DestTilesPerDay
    {
      get
      {
        if (destTilesPerDayDirty)
        {
          ...
          cachedDestTilesPerDay = TilesPerDayCalculator.ApproxTilesPerDay(transferables,
            DestMassUsage, DestMassCapacity, caravan.Tile,
            (!caravan.vehiclePather.Moving) ? PlanetTile.Invalid : caravan.vehiclePather.nextTile,
            false, explanation: explanation);
          cachedDestTilesPerDayExplanation = explanation.ToString();
        }
    ...
    private Pair<float, float> DestDaysWorthOfFood
    {
      get
      {
        if (destDaysWorthOfFoodDirty)
        {
          ...
          {
            first = DaysWorthOfFoodCalculator.ApproxDaysWorthOfFood(transferables, caravan.Tile,
              IgnorePawnsInventoryMode.Ignore, caravan.Faction, caravan.vehiclePather.curPath,
              caravan.vehiclePather.nextTileCostLeft, caravan.TicksPerMove);
            second = DaysUntilRotCalculator.ApproxDaysUntilRot(transferables, caravan.Tile,
              IgnorePawnsInventoryMode.Ignore, caravan.vehiclePather.curPath,
              caravan.vehiclePather.nextTileCostLeft, caravan.TicksPerMove);
          }
          else
          {
            first = DaysWorthOfFoodCalculator.ApproxDaysWorthOfFood(transferables, caravan.Tile,
              IgnorePawnsInventoryMode.Ignore, caravan.Faction, null, 0f, 3300);
            second = DaysUntilRotCalculator.ApproxDaysUntilRot(transferables, caravan.Tile,
              IgnorePawnsInventoryMode.Ignore, null, 0f, 3300);
          }
    ...
    private float DestVisibility
    {
      get
      {
        if (destVisibilityDirty)
        {
          ...
          StringBuilder stringBuilder = new StringBuilder();
          cachedDestVisibility =
            CaravanVisibilityCalculator.Visibility(transferables, stringBuilder);
    ...
    public override void DoWindowContents(Rect inRect)
    {
      ...
      {
        CaravanUIUtility.DrawCaravanInfo(new CaravanUIUtility.CaravanInfo(SourceMassUsage,
            SourceMassCapacity, cachedSourceMassCapacityExplanation, SourceTilesPerDay,
            cachedSourceTilesPerDayExplanation, SourceDaysWorthOfFood, SourceForagedFoodPerDay,
            cachedSourceForagedFoodPerDayExplanation, SourceVisibility,
            cachedSourceVisibilityExplanation, -1f, -1f, null), null, caravan.Tile,
          (!caravan.vehiclePather.Moving) ? null : new int?(TicksToArrive), -9999f,
    ...
    {
      Rect acceptButtonRect = new Rect(rect.width / 2f - BottomButtonSize.x / 2f,
        rect.height - BottomAreaHeight, BottomButtonSize.x, BottomButtonSize.y);
      if (Widgets.ButtonText(acceptButtonRect, "AcceptButton".Translate(), true, false, true) &&
        TransferPawns())
      {
        SoundDefOf.Tick_High.PlayOneShotOnCamera(null);
        Close(false);
      }
      Rect resetButtonRect = new Rect(acceptButtonRect.x - 10f - BottomButtonSize.x,
        acceptButtonRect.y, BottomButtonSize.x, BottomButtonSize.y);
      if (Widgets.ButtonText(resetButtonRect, "ResetButton".Translate(), true, false, false))
      {
        SoundDefOf.Tick_Low.PlayOneShotOnCamera(null);
        CalculateAndRecacheTransferables();
      }
      Rect cancelButtonRect = new Rect(acceptButtonRect.xMax + 10f, acceptButtonRect.y,
    ...
    private void CalculateAndRecacheTransferables()
    {
      ...
      CreateCaravanItemsWidget(transferables, out itemsTransfer,
        "SplitCaravanThingCountTip".Translate(), IgnorePawnsInventoryMode.Ignore,
        () => DestMassCapacity - DestMassUsage,
    ...
    private void CreateCaravanItemsWidget(List<TransferableOneWay> transferables,
      out TransferableOneWayWidget itemsTransfer, string thingCountTip,
      IgnorePawnsInventoryMode ignorePawnInventoryMass,
      Func<float> availableMassGetter, bool ignoreSpawnedCorpsesGearAndInventoryMass, int tile,
      bool playerPawnsReadOnly = false)
    {
      itemsTransfer = new TransferableOneWayWidget(transferables, null, null, thingCountTip, true,
        ignorePawnInventoryMass, false, availableMassGetter, 0f,
        ignoreSpawnedCorpsesGearAndInventoryMass, tile,
        true, false, false, true, false, true, false);
    }

    private bool CheckForErrors(List<Pawn> pawns)
    {
      if (!pawns.NotNullAndAny((Pawn x) =>
        CaravanUtility.IsOwner(x, Faction.OfPlayer) && !x.Downed))
      {
        Messages.Message("CaravanMustHaveAtLeastOneColonist".Translate(), caravan,
