...
namespace Vehicles
{
  ...
  public class Dialog_ColorPicker : Window
  {
    ...
    public static void OpenColorPicker(VehiclePawn vehicle,
      Action<Color, Color, Color, PatternDef, Vector2, float> onSave)
    {
      Dialog_ColorPicker colorPicker = new(vehicle)
      {
        OnSave = onSave,
    ...
    /// <param name="onSave"></param>
    public static void OpenColorPicker(VehicleDef vehicleDef,
      Action<Color, Color, Color, PatternDef, Vector2, float> onSave)
    {
      Dialog_ColorPicker colorPicker = new(vehicleDef)
      {
        OnSave = onSave,
        PatternData =
          new PatternData(
    ...
    private void RecacheAvailablePatterns()
    {
      if (showPatterns)
      {
        AvailablePatterns = DefDatabase<PatternDef>.AllDefsListForReading.Where(patternDef =>
          !(patternDef is SkinDef) && patternDef.ValidFor(VehicleDef)).ToList();
      }
      else
      {
    ...
    public override void DoWindowContents(Rect inRect)
    {
        ...
        "VF_ResetColorPalettes".Translate()))
      {
        Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(
          ...
          delegate() { VehicleMod.settings.colorStorage.ResetPalettes(); }, false, null));
      }
      if (VehicleDef.graphicData.drawRotated &&
        VehicleDef.graphicData.Graphic is Graphic_Vehicle graphicVehicle)
      {
        Rect rotateVehicleRect = new Rect(inRect.width / 3 - 10 - ButtonHeight, 0, ButtonHeight,
          ButtonHeight);
        ...
        if (Widgets.ButtonInvisible(rotateVehicleRect))
        {
          ...
          for (int i = 0; i < 4; i++)
          {
            ...
            if (validRotations.Contains(DisplayRotation.Value))
            {
              break;
            }
          }
      ...
      Rect paintRect = new Rect(inRect.width / 3f - 5f, colorContainerRect.y, inRect.width / 3,
        topBoxHeight);
      ...
      Rect paletteRect = new Rect(inRect.width / 3f - 5f, inRect.height - panelHeight, panelWidth,
        panelHeight);
      DrawColorPalette(paletteRect);

      Rect displayRect = new Rect(0,
        paintRect.y + ((paintRect.height / 2) - ((paintRect.width - 15) / 2)), paintRect.width - 15,
        paintRect.width - 15);
      Widgets.BeginGroup(displayRect);
      {
        Rect vehicleDefRect = displayRect.AtZero();
        HandleDisplacementDrag(vehicleDefRect);
        PatternData patternData = new PatternData(currentColorOne.ToColor, currentColorTwo.ToColor,
          currentColorThree.ToColor, selectedPattern, new Vector2(displacementX, displacementY),
          additionalTiling);
        if (Vehicle != null) //TODO - render vehicle instance
        {
          VehicleGraphics.DrawVehicle(vehicleDefRect, Vehicle, patternData, DisplayRotation);
        }
        else
        {
          VehicleGraphics.DrawVehicleDef(vehicleDefRect, VehicleDef, patternData, DisplayRotation,
            withoutTurrets: true);
        }
      }
      Widgets.EndGroup();
      ...
      Rect sliderRect =
        new Rect(0f, inRect.height - SliderHeight * 3, ButtonWidth * 3, SliderHeight);
      ...
      UIElements.SliderLabeled(positionLeftBox, "VF_PatternDisplacementX".Translate(),
        string.Empty,
        string.Empty, ref displacementX, -1.5f, 1.5f);
      UIElements.SliderLabeled(positionRightBox, "VF_PatternDisplacementY".Translate(),
        string.Empty, string.Empty, ref displacementY, -1.5f, 1.5f);
    ...
    private void DrawPaintSelection(Rect paintRect)
    {
      ...
      Rect switchRect = new Rect(paintRect.x, paintRect.y, paintRect.width / 2 - IconButtonSize / 2,
        labelHeight);
      ...
      Rect toggleRect = new Rect(switchRect.xMax, switchRect.y - IconButtonSize / 4 - 2,
      ...
      int maxIndex = Ext_Numeric.Clamp(pageNumber * (GridDimensionColumns * GridDimensionRows), 0,
        AvailablePatterns.Count);
      int iteration = 0;
      Rect displayRect = new Rect(0, 0, gridSize, gridSize);
      Rect paginationRect = new Rect(paintRect.x + 5, paintRect.y + paintRect.height - ButtonHeight,
      ...
      for (int i = startingIndex; i < maxIndex; i++, iteration++)
      {
        ...
        Widgets.BeginGroup(displayRect);
        {
          // TODO - Allow rendering of vehicle instance for color picker
          if (Vehicle != null)
          {
            VehicleGraphics.DrawVehicle(displayRect.AtZero(), Vehicle, patternData, DisplayRotation,
              withoutTurrets: true);
          }
          else
          {
            VehicleGraphics.DrawVehicleDef(displayRect.AtZero(), VehicleDef, patternData,
              DisplayRotation, withoutTurrets: true);
          }
        }
        Widgets.EndGroup();
    ...
    private void DrawColorSelection(Rect colorContainerRect)
    {
      ...
      Rect colorPickerRect =
        RenderHelper.DrawColorPicker(colorRect, ref hue, ref saturation, ref value, SetColor);
      ...
      Rect reverseRect = new Rect(colorContainerRect.x + 11f, 20, SwitchSize / 2.75f,
        SwitchSize / 2.75f);
      ...
      Rect inputRect = new Rect(colorRect.x, colorRect.y + colorRect.height + 5f,
    ...
    private void DrawColorPalette(Rect rect)
    {
      ...
      float rectSize =
        (rect.width - (ColorStorage.PaletteCount / ColorStorage.PaletteRowCount - 1) * 5f) /
      ...
      for (int i = 0; i < ColorStorage.PaletteCount; i++)
      {
        ...
        Rect buttonRect = new Rect(displayRect.x, displayRect.y, displayRect.width * 3,
          displayRect.height);
      ...
      switch (colorSelected)
      {
        ...
        default:
          throw new ArgumentOutOfRangeException(
            "ColorSelection out of range. Must be between 1 and 3.");
      }
    }
