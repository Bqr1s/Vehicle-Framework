    ...
    public override void DoWindowContents(Rect inRect)
    {
      ...
      if (!vehicleDef.vehicleStats.NullOrEmpty())
      {
        Dictionary<string, float> stats =
          VehicleMod.settings.vehicles.vehicleStats.TryGetValue(vehicleDef.defName, null);
        foreach (VehicleStatModifier statModifier in VehicleMod.selectedDef.vehicleStats)
        {
          ...
          float value = stats?.TryGetValue(statModifier.statDef.defName, statModifier.value) ??
            statModifier.value;
          ...
          switch (settingsType)
          {
            case UISettingsType.IntegerBox:
            {
              ...
              listing.FloatBox(label, ref newValue, string.Empty, string.Empty,
                min: statModifier.statDef.minValue, max: statModifier.statDef.maxValue,
                lineHeight: 24, labelProportion: 0.5f);
              if (value != newValue)
              {
                if (!VehicleMod.settings.vehicles.vehicleStats.ContainsKey(vehicleDef.defName))
                {
                  VehicleMod.settings.vehicles.vehicleStats[vehicleDef.defName] =
                    new Dictionary<string, float>();
                }
                VehicleMod.settings.vehicles.vehicleStats[vehicleDef.defName][
                  statModifier.statDef.defName] = Mathf.RoundToInt(newValue);
              }
            }
              break;
            case UISettingsType.FloatBox:
            {
              ...
              if (value != newValue)
              {
                ...
                VehicleMod.settings.vehicles.vehicleStats[vehicleDef.defName][
                  statModifier.statDef.defName] = newValue;
              }
            }
              break;
            case UISettingsType.SliderFloat:
            {
              ...
              listing.SliderLabeled(label, ref newValue, string.Empty, string.Empty, string.Empty,
                min: statModifier.statDef.minValue, max: statModifier.statDef.maxValue,
                decimalPlaces: GetRoundingPlaces(statModifier.statDef.toStringStyle));
              if (value != newValue)
              {
                if (!VehicleMod.settings.vehicles.vehicleStats.ContainsKey(vehicleDef.defName))
                {
          ...
          if (Mouse.IsOver(currentRect))
          {
            ...
            if (Event.current.type == EventType.MouseDown && Event.current.button == 1)
            {
              ...
              options.Add(new FloatMenuOption("ResetButton".Translate(), delegate()
              {
                ...
                {
                  VehicleMod.settings.vehicles.vehicleStats[vehicleDef.defName]
                   .Remove(statModifier.statDef.defName);
                }
              }));
