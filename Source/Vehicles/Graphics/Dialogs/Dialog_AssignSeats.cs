...
namespace Vehicles
{
  public class Dialog_AssignSeats : Window
  {
    ...
    private readonly Dictionary<Pawn, (VehiclePawn vehicle, VehicleRoleHandler handler)>
      assignedSeats = new Dictionary<Pawn, (VehiclePawn, VehicleRoleHandler)>();
    ...
    public Dialog_AssignSeats(List<TransferableOneWay> pawns, TransferableOneWay transferable)
    {
      ...
      {
        if (assignedSeats.TryGetValue(pawn,
            out (VehiclePawn vehicle, VehicleRoleHandler handler) assignedSeat) &&
          assignedSeat.vehicle == Vehicle)
        {
    ...
    private void DrawPawnRow(Rect rect, Pawn pawn,
      (float width, string label, Action onClick) button)
    {
      Rect colonistIconRect = new Rect(rect.x, rect.y, RowHeight, RowHeight);
      Rect colonistRect = new Rect(colonistIconRect.x + RowHeight, rect.y + 5, rect.width - 1,
        RowHeight);
      Widgets.Label(colonistRect, pawn.LabelCap);
      Widgets.ThingIcon(colonistIconRect, pawn);

      Rect buttonRect = new Rect(rect.x + colonistRect.width - 100, rect.y + 5, button.width,
        ButtonHeight - 10);
    ...
    private void DrawPawns(Rect rect)
    {
      ...
      Widgets.BeginScrollView(outRect, ref dialogPawnsScrollPos, viewRect);
      {
        foreach (Pawn pawn in pawns.Where(a => !assignedSeats.ContainsKey(a)))
        {
          ...
          Rect entryButtonRect = new Rect(pawnRowRect.x + pawnRowRect.width - (buttonWidth + 10),
            pawnRowRect.y + 5, buttonWidth, ButtonHeight - 10);
          ...
          if (draggedPawn != pawn)
          {
            DrawPawnRow(pawnRowRect, pawn, (ButtonHeight * 3, "VF_AddToRole".Translate(), delegate()
                {
                  bool Validate(VehicleRoleHandler handler) =>
                    assignedSeats.Where(seat => seat.Value.handler.role == handler.role)
                     .Select(p => p.Key).Count() < handler.role.Slots;

                  VehicleRoleHandler firstHandler = Vehicle.handlers.FirstOrDefault(handler =>
                    handler.CanOperateRole(pawn) && Validate(handler));
                  firstHandler ??= Vehicle.handlers.FirstOrDefault(handler =>
                    !handler.RequiredForMovement && Validate(handler));
                  if (firstHandler != null)
                  {
                    if (!firstHandler.CanOperateRole(pawn))
                    {
                      if (firstHandler.role.HandlingTypes.HasFlag(HandlingType.Movement))
                      {
    ...
    private void DrawAssignees(Rect rect)
    {
      ...
      Predicate<VehicleRoleHandler> seatsAvailable = (handler) =>
        assignedSeats.Where(seat => seat.Value.handler.role == handler.role).Select(p => p.Key)
         .Count() < handler.role.Slots;
      Rect pawnRowRect = new Rect(rect.x, rect.y + RowHeight + 5, rect.width - 1, RowHeight);
      Rect outRect = new Rect(pawnRowRect)
      {
        x = rect.x,
      ...
      {
        width = outRect.width - 17,
        height = (Vehicle.handlers.Count + assignedSeats.Count) * RowHeight +
          Vehicle.handlers.Where(handler => seatsAvailable(handler)).Count() * RowHeight + 5
      };
      Widgets.BeginScrollView(outRect, ref dialogPawnsAssignedScrollPos, viewRect);
      {
        foreach (VehicleRoleHandler handler in Vehicle.handlers)
        {
          int seatsOccupied = assignedSeats.Where(r => r.Value.handler.role == handler.role)
          ...
          UIElements.LabelUnderlined(pawnRowRect, handler.role.label,
            $"({handler.role.Slots - assignedSeats.Where(r => r.Value.handler.role == handler.role).Select(p => p.Key).Count()})",
            Color.white, countColor, Color.white);
          pawnRowRect.y += RowHeight;

          Rect roleRect = new Rect(pawnRowRect.x, pawnRowRect.y, pawnRowRect.width,
            RowHeight + RowHeight * assignedSeats.Where(r => r.Value.handler.role == handler.role)
             .Select(p => p.Key).Count());
          ...
          if (open && Mouse.IsOver(roleRect) && draggedPawn != null)
          {
            if (Event.current.type == EventType.MouseUp && Event.current.button == 0)
            {
              if (handler.role.HandlingTypes > HandlingType.None &&
                !draggedPawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation) ||
                draggedPawn.Downed || draggedPawn.Dead)
              {
          ...
          foreach ((Pawn pawn, (VehiclePawn vehicle, VehicleRoleHandler assignedHandler)) in
            assignedSeats.Where(r => r.Value.handler.role == handler.role))
          {
            Action onClick = null;
            ...
            DrawPawnRow(pawnRowRect, pawn,
              (ButtonHeight * 3, "VF_RemoveFromRole".Translate(), onClick));
            pawnRowRect.y += RowHeight;
          }
    ...
    {
      Rect buttonRect = new Rect(rect.width - ButtonWidth * 3, rect.height - ButtonHeight,
      ...
      {
        SoundDefOf.Click.PlayOneShotOnCamera(null);
        assignedSeats.RemoveAll(kvp =>
          pawns.Contains(kvp.Key) && !Vehicle.AllPawnsAboard.Contains(kvp.Key));
      }
    }
    ...
    private bool FinalizeSeats(out string failReason)
    {
      ...
      foreach (VehicleRoleHandler handler in Vehicle.handlers.Where(x => x.role.RequiredForCaravan))
      {
        if (assignedSeats.Where(r => r.Value.handler.role == handler.role).Select(k => k.Key)
           .Count() < handler.role.SlotsToOperate)
        {
      ...
      try
      {
        CaravanHelper.ClearAssignedSeats(Vehicle,
          (Pawn pawn) => transferablePawns.FirstOrDefault(p => (p.AnyThing as Pawn) == pawn)
          ?.ForceTo(0));
        ...
        foreach ((Pawn pawn, (VehiclePawn vehicle, VehicleRoleHandler handler) assignment) in
          assignedSeats)
        {
      ...
      List<TransferableOneWay> outList)
    {
      ...
      foreach (TransferableOneWay transferable in pawns)
      {
        ...
        {
          if (!CaravanHelper.assignedSeats.ContainsKey(pawn) ||
            CaravanHelper.assignedSeats[pawn].vehicle == vehicle)
          {
