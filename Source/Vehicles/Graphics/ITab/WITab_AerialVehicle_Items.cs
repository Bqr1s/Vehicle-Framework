...
namespace Vehicles
{
  public class WITab_AerialVehicle_Items : WITab_AerialVehicle
  {
    ...
    protected override void FillTab()
    {
      ...
      TransferableUIUtility.DoTransferableSorters(sorter1, sorter2,
      ...
      CheckCacheItems();
      AerialVehicleTabHelper.DoRows(rect.size, cachedItems, SelAerialVehicle, ref scrollPosition,
    ...
    private void CacheItems()
    {
      ...
      for (int i = 0; i < list.Count; i++)
      {
        TransferableImmutable transferableImmutable =
          TransferableUtility.TransferableMatching(list[i], cachedItems, TransferAsOneMode.Normal);
      ...
      cachedItems = cachedItems.OrderBy((TransferableImmutable tr) => tr, sorter1.Comparer)
       .ThenBy((TransferableImmutable tr) => tr, sorter2.Comparer)
       .ThenBy((TransferableImmutable tr) => TransferableUIUtility.DefaultListOrderPriority(tr))
       .ToList<TransferableImmutable>();
