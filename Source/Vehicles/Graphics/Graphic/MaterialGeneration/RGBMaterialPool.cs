  ...
  public static class RGBMaterialPool
  {
    ...
    public static Material Get(IMaterialCacheTarget target, Rot8 rot)
    {
      if (cache.TryGetValue(target, out Material[] materials))
      {
        ...
        {
          Log.Error($@"Attempting to fetch material out of bounds. Target={target} Rot8={rot}.  
Max count for {target} is {target.MaterialCount}");
    ...
    public static void CacheMaterialsFor(IMaterialCacheTarget target, PatternDef patternDef, int renderQueue = 0, 
      List<ShaderParameter> shaderParameters = null)
    {
    ...
    public static void Release(IMaterialCacheTarget target)
    {
      if (cache.TryGetValue(target, out Material[] materials))
      {
        ...
        Debug.Message($@"<success>{VehicleHarmony.LogLabel}</success> Removed {target} from RGBMaterialPool and 
cleared all entries.");
      }
    }
...
{((Dictionary<Material, MaterialRequest>)AccessTools.Field(typeof(MaterialPool), "matDictionaryReverse")
.GetValue(null)).Count}");
