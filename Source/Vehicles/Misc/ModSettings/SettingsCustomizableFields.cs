...
public static class SettingsCustomizableFields
{
  ...
  public static bool PopulateSaveableFields(VehicleDef def, bool hardReset = false)
  {
    try
    {
      ...
      {
        VehicleMod.settings.vehicles.fieldSettings[def.defName] =
          new Dictionary<SaveableField, SavedField<object>>();
        currentDict = VehicleMod.settings.vehicles.fieldSettings[def.defName];
      }
      VehicleMod.settings.vehicles.defaultValues[def.defName] =
        new Dictionary<SaveableField, object>();
  ...
  public static bool PopulateSaveableUpgrades(VehicleDef def, bool hardReset = false)
  {
    try
    {
      ...
      {
        if (!VehicleMod.settings.upgrades.upgradeSettings.TryGetValue(def.defName,
          out var currentUpgradeDict))
        {
          VehicleMod.settings.upgrades.upgradeSettings.Add(def.defName,
            new Dictionary<SaveableField, SavedField<object>>());
          currentUpgradeDict = VehicleMod.settings.upgrades.upgradeSettings[def.defName];
          foreach (UpgradeNode node in def.GetSortedCompProperties<CompProperties_UpgradeTree>().def
           .nodes)
          {
