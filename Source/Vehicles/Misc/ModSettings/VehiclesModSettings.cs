  ...
  public class VehiclesModSettings : ModSettings
  {
    ...
    public override void ExposeData()
    {
      try
      {
        ...
        Scribe_Values.Look(ref promptVehicleCollision, nameof(promptVehicleCollision),
          defaultValue: true);
      }
      catch (Exception ex)
      {
        Log.Error(
          $"Exception thrown while trying to load mod settings. Deleting the Vehicles config file might fix this.\nException={ex}\nInnerException={ex.InnerException}");
      }
    }
    ...
    public static void TryPromptPlayer(string labelKey, string descriptionKey, Action onConfirm,
      Action onDeny)
    {
    ...
    public static void OpenWithContext(SettingsSection section = null)
    {
      ...
      else if (!WorldRendererUtility.WorldRendered && Find.CurrentMap != null &&
        Find.Selector.SelectedObjects.FirstOrDefault() is VehiclePawn vehicle)
      {
