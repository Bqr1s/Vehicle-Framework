  ...
  public class PostToSettingsAttribute : Attribute
  {
    ...
    public void DrawLister(Listing_Settings lister, VehicleDef vehicleDef, FieldInfo field)
    {
      ...
      if (field.TryGetAttribute(out DisableSettingConditionalAttribute disableSetting))
      {
        ...
        {
          disabledTooltip =
            "VF_SaveableFieldDisabledConditionTooltip".Translate(fieldDisabledTooltip);
        }
        else if (disableSetting.PropertyDisabled(vehicleDef, out string propertyDisabledTooltip))
        {
          disabledTooltip =
            "VF_SaveableFieldDisabledConditionTooltip".Translate(propertyDisabledTooltip);
        }
      }
      ...
      bool locked = false;
      if (ParsingHelper.lockedFields.TryGetValue(vehicleDef.defName,
        out HashSet<FieldInfo> lockedFields))
      {
      ...
      if (field.FieldType.GetInterface(nameof(ICustomSettingsDrawer)) is ICustomSettingsDrawer
        settingsDrawer)
      {
        settingsDrawer.DrawSetting(lister, vehicleDef, field, label, tooltip, disabledTooltip,
          locked, Translate);
      }
      else
      {
        DrawSetting(lister, vehicleDef, field, UISettingsType, label, tooltip, disabledTooltip,
          locked, Translate);
      }
    }

    public static void DrawSetting(Listing_Settings lister, VehicleDef vehicleDef, FieldInfo field,
      UISettingsType settingsType, string label, string tooltip, string disabledTooltip,
      bool locked, bool translate)
    {
      ...
      switch (settingsType)
      {
        ...
        case UISettingsType.IntegerBox:
        {
          if (field.TryGetAttribute<NumericBoxValuesAttribute>(out var inputBox))
          {
            lister.IntegerBox(vehicleDef, saveable, label, tooltip, disabledTooltip,
              Mathf.RoundToInt(inputBox.MinValue), Mathf.RoundToInt(inputBox.MaxValue));
          }
          else
          {
            lister.IntegerBox(vehicleDef, saveable, label, tooltip, disabledTooltip, 0,
              int.MaxValue);
          }
          break;
        }
        case UISettingsType.FloatBox:
        {
          if (field.TryGetAttribute<NumericBoxValuesAttribute>(out var inputBox))
          {
            lister.FloatBox(vehicleDef, saveable, label, tooltip, disabledTooltip,
              inputBox.MinValue, inputBox.MaxValue);
          }
          else
          {
            lister.FloatBox(vehicleDef, saveable, label, tooltip, disabledTooltip, 0,
              float.MaxValue);
          }
          break;
        }
        ...
        case UISettingsType.SliderInt:
        {
          if (field.TryGetAttribute<SliderValuesAttribute>(out var slider))
          {
            lister.SliderLabeled(vehicleDef, saveable, label, tooltip, disabledTooltip,
              slider.EndSymbol, (int)slider.MinValue, (int)slider.MaxValue, (int)slider.EndValue,
              slider.MaxValueDisplay, slider.MinValueDisplay, translate);
          }
          else
          {
            Log.WarningOnce(
              $"Slider declared for SaveableField {field.Name} in {field.DeclaringType} with no SliderValues attribute. Slider will use default values instead.",
              field.GetHashCode());
            lister.SliderLabeled(vehicleDef, saveable, label, tooltip, disabledTooltip,
        ...
        case UISettingsType.SliderFloat:
        {
          ...
          {
            lister.SliderLabeled(vehicleDef, saveable, label, tooltip, disabledTooltip,
              slider.EndSymbol, slider.MinValue, slider.MaxValue, slider.RoundDecimalPlaces,
              slider.EndValue, slider.Increment, slider.MaxValueDisplay, translate);
          }
          else
          {
            Log.WarningOnce(
        ...
        case UISettingsType.SliderPercent:
        {
          if (field.TryGetAttribute<SliderValuesAttribute>(out var slider))
          {
            lister.SliderPercentLabeled(vehicleDef, saveable, label, tooltip, disabledTooltip,
              slider.EndSymbol, slider.MinValue, slider.MaxValue, slider.RoundDecimalPlaces,
              slider.EndValue, slider.MaxValueDisplay, translate);
          }
          else
          {
            Log.WarningOnce(
    ...
    public static void DrawSetting(Listing_Settings lister, VehicleDef vehicleDef, FieldInfo field,
      SettingsValueInfo settingsInfo, string label, string tooltip, string disabledTooltip,
      bool locked, bool translate)
    {
      SaveableField saveable = new SaveableField(vehicleDef, field);
      switch (settingsInfo.settingsType)
      {
        ...
        {
          lister.IntegerBox(vehicleDef, saveable, label, tooltip, disabledTooltip,
            Mathf.RoundToInt(settingsInfo.minValue), Mathf.RoundToInt(settingsInfo.maxValue));
          break;
        }
        case UISettingsType.FloatBox:
        {
          lister.FloatBox(vehicleDef, saveable, label, tooltip, disabledTooltip,
            settingsInfo.minValue, settingsInfo.maxValue);
          break;
        }
        case UISettingsType.SliderEnum:
          lister.EnumSliderLabeled(vehicleDef, saveable, label, tooltip, disabledTooltip,
        ...
        {
          lister.SliderLabeled(vehicleDef, saveable, label, tooltip, disabledTooltip,
            settingsInfo.endSymbol, (int)settingsInfo.minValue, (int)settingsInfo.maxValue,
        ...
        {
          lister.SliderLabeled(vehicleDef, saveable, label, tooltip, disabledTooltip,
            settingsInfo.endSymbol, settingsInfo.minValue, settingsInfo.maxValue,
