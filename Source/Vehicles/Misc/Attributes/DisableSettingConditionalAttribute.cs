...
public class DisableSettingConditionalAttribute : Attribute
{
  ...
  public bool PropertyDisabled(VehicleDef vehicleDef, out string tooltip)
  {
    ...
    {
      Log.Warning(
        "DisableSettingConditional has Property name, not null MemberType. MemberType property must be included for PropertyInfo to be evaluated.");
      return false;
    }
    PropertyInfo propertyInfo = AccessTools.Property(MemberType, Property);
    if (propertyInfo == null || propertyInfo.GetGetMethod() == null)
    {
      Log.Error($"MemberInfo for {MemberType}.{Property} not found.");
      return false;
    }
  ...
  public bool FieldDisabled(VehicleDef vehicleDef, out string tooltip)
  {
    ...
    {
      Log.Warning(
        "DisableSettingConditional has Field name, not null MemberType. MemberType property must be included for FieldInfo to be evaluated.");
      return false;
    }
  ...
  private static string BuildDisabledReport(MemberInfo memberInfo, string comparisonLabel,
    object value)
  {
    string memberName = memberInfo.Name;
    if (memberInfo.TryGetAttribute(out PostToSettingsAttribute attribute))
    {
  ...
  private static object IterateTypesForParent(object parent, MemberInfo memberInfo)
  {
    ...
    foreach (FieldInfo fieldInfo in parent.GetType()
