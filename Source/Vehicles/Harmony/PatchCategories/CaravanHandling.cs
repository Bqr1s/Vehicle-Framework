...
namespace Vehicles
{
  internal class CaravanHandling : IPatchCategory
  {
    ...
    public void PatchMethods()
    {
      ...
      VehicleHarmony.Patch(
        original: AccessTools.Method(typeof(CaravanArrivalAction_VisitEscapeShip),
          "DoArrivalAction"), prefix: null, postfix: null,
        transpiler: new HarmonyMethod(typeof(CaravanHandling),
          nameof(ShipsVisitEscapeShipTranspiler)));
      VehicleHarmony.Patch(original: AccessTools.Method(typeof(SettlementUtility), "AttackNow"),
    ...
    /// </summary>
    public static bool CapacityOfVehicle(Pawn p, ref float __result,
    ...
    public static IEnumerable<CodeInstruction> PawnCapacityInVehicleTranspiler(
      ...
      for (int i = 0; i < instructionList.Count; i++)
      {
        ...
        if (instruction.Calls(capacityMethod))
        {
          yield return new CodeInstruction(opcode: OpCodes.Call,
            operand: AccessTools.Method(typeof(CaravanHandling), nameof(PawnCapacityInVehicle)));
    ...
    public static IEnumerable<CodeInstruction> IgnorePawnGearAndInventoryMassTranspiler(
      IEnumerable<CodeInstruction> instructions)
    {
      List<CodeInstruction> instructionList = instructions.ToList();
      MethodInfo capacityMethod =
      ...
      for (int i = 0; i < instructionList.Count; i++)
      {
        ...
        if (instruction.Calls(capacityMethod))
        {
          ...
          yield return new CodeInstruction(opcode: OpCodes.Call,
            operand: AccessTools.Method(typeof(CaravanHandling), nameof(PawnMassUsageInVehicle)));
        }
    ...
    public static bool FillTabVehicleCaravan(ITab_Pawn_FormingCaravan __instance,
    ...
    public static IEnumerable<CodeInstruction> ShipsVisitEscapeShipTranspiler(
    ...
    public static IEnumerable<CodeInstruction> AttackNowWithShipsTranspiler(
      IEnumerable<CodeInstruction> instructions, ILGenerator ilg)
    {
      List<CodeInstruction> instructionList = instructions.ToList();
    ...
    {
      if (thing is Pawn && thing.ParentHolder is VehicleHandler handler &&
    ...
    public static IEnumerable<CodeInstruction> MergeWithVehicleCaravanTranspiler(
      IEnumerable<CodeInstruction> instructions)
    {
    ...
    public static void NoTradingUndocked(Caravan caravan, ref FloatMenuAcceptanceReport __result)
    {
