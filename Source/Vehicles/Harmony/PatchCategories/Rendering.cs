      ...
      for (int i = 0; i < instructionList.Count; i++)
      {
        ...
        if (instruction.Calls(clearCachedEntriesMethod))
        {
          ...
          yield return new CodeInstruction(opcode: OpCodes.Call,
            operand: AccessTools.Method(typeof(Rendering), nameof(RecacheAerialVehicleEntries)));
        }
      ...
      CellRect viewRect)
    {
      if (__instance is VehiclePawn vehicle)
      {
        if (!vehicle.Suspended && vehicle.Spawned)
        {
          ...
          {
            vehicle.DrawTracker.ProcessPostTickVisuals(ticksPassed);
          }
      ...
      IEnumerable<CodeInstruction> instructions)
    {
      ...
      for (int i = 0; i < instructionList.Count; i++)
      {
          ...
          instructionList[i - 1].Calls(TrueCenter_Baseline))
        {
          ...
          yield return new CodeInstruction(opcode: OpCodes.Call,
            operand: AccessTools.Method(typeof(Rendering),
              nameof(Rendering.VehicleTrueCenterReroute)));
        }
