...
namespace Vehicles
{
  internal class Combat : IPatchCategory
  {
    public void PatchMethods()
    {
      ...
      VehicleHarmony.Patch(
        original: AccessTools.PropertyGetter(typeof(Projectile), nameof(Projectile.HitFlags)),
        postfix: new HarmonyMethod(typeof(Combat),
    ...
    public static void StartingTicksFromTurret(Projectile __instance, ref float __result,
      Vector3 ___origin, Vector3 ___destination)
    {
      if (__instance.TryGetComp<CompTurretProjectileProperties>() is CompTurretProjectileProperties
        comp)
      {
    ...
    /// <param name="__result"></param>
    public static bool TurretHitFlags(Thing thing, Projectile __instance, Thing ___launcher,
      ref bool __result)
    {
      if (__instance.TryGetComp<CompTurretProjectileProperties>() is CompTurretProjectileProperties
        comp)
      {
        ...
        foreach (IntVec3 c in thing.OccupiedRect())
        {
          ...
          {
            if (thingList[i] != thing && ((comp.hitflags != null &&
                  thingList[i].def.fillPercent >= comp.hitflags.minFillPercent) ||
                (comp.hitflags is null && thingList[i].def.Fillage == FillCategory.Full)) &&
              thingList[i].def.Altitude >= thing.def.Altitude)
            {
              flag2 = true;
              break;
            }
        ...
        if (thing == __instance.intendedTarget && (hitFlags & ProjectileHitFlags.IntendedTarget) !=
        ...
        if (thing != __instance.intendedTarget)
        {
          if (thing is Pawn pawn)
          {
            ...
            if (comp.hitflags != null && comp.hitflags.hitThroughPawns && !pawn.Dead &&
              !pawn.Downed)
            {
              thing.TakeDamage(new DamageInfo(DamageDefOf.Blunt, comp.speed * 2, 0, -1,
                __instance));
            }
          }
    ...
    public static bool ImpactExplosiveProjectiles(Thing hitThing, Projectile __instance,
      Thing ___launcher)
    {
      if (hitThing is VehiclePawn vehicle)
      {
        ...
        __instance.def.GetModExtension<ReduceExplosionOnWater>() != null)
      {
        ...
        if (__instance.def.projectile.explosionDelay == 0 && terrainImpact.IsWater && !__instance
         .Position.GetThingList(__instance.Map).NotNullAndAny(x => x is VehiclePawn vehicle))
        {
    ...
    public static IEnumerable<CodeInstruction> VehicleProjectileChanceToHit(
      IEnumerable<CodeInstruction> instructions)
    {
      ...
      for (int i = 0; i < instructionList.Count; i++)
      {
        ...
        if (instruction.opcode == OpCodes.Stloc_S &&
          instruction.operand is LocalBuilder localBuilder && localBuilder.LocalIndex == 8)
        {
          ...
          yield return new CodeInstruction(opcode: OpCodes.Call,
            AccessTools.Method(typeof(Combat), nameof(VehiclePawnFillageInterceptReroute)));
        }
    ...
    public static void ProjectileMapToWorld(Thing __instance, DestroyMode mode = DestroyMode.Vanish)
    {
      if (__instance is Projectile projectile &&
        projectile.GetComp<CompProjectileExitMap>() is CompProjectileExitMap exitMap)
      {
        exitMap.LeaveMap();
      }
    }

    public static IEnumerable<CodeInstruction> VehicleProjectileInterceptor(
      IEnumerable<CodeInstruction> instructions)
    {
      List<CodeInstruction> instructionList = instructions.ToList();
    ...
    {
      if (__instance.Map.GetCachedMapComponent<VehiclePositionManager>().ClaimedBy(c) is VehiclePawn
        vehicle)
      {
    ...
    public static void VehicleMultipleExplosionInstances(Thing t, ref List<Thing> damagedThings,
    ...
    public static IEnumerable<CodeInstruction> VehicleExplosionDamageTranspiler(
      IEnumerable<CodeInstruction> instructions, ILGenerator ilg)
    {
      ...
      for (int i = 0; i < instructionList.Count; i++)
      {
        ...
        if (instruction.Calls(takeDamageMethod))
        {
          ...
          yield return new CodeInstruction(opcode: OpCodes.Call,
            operand: AccessTools.Method(typeof(Combat), nameof(Combat.TakeDamageReroute)));
