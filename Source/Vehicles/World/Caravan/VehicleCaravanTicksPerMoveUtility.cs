...
namespace Vehicles
{
  public static class VehicleCaravanTicksPerMoveUtility
  {
    ...
    public static int TicksFromMoveSpeed(float moveSpeed)
    {
    ...
    private static void AppendUsingDefaultTicksPerMoveInfo(StringBuilder explanation)
    {
      ...
      explanation.AppendLine();
      explanation.Append(
        $"  {"Default".Translate()}: {18.181818f:0.#} {"TilesPerDay".Translate()}");
    }
    ...
    public static float ApproxTilesPerDay(List<VehicleDef> vehicleDefs, int ticksPerMove, int tile,
      int nextTile, StringBuilder explanation = null, string caravanTicksPerMoveExplanation = null)
    {
      if (nextTile == -1)
      {
        nextTile = Find.WorldGrid.FindMostReasonableAdjacentTileForDisplayedPathCost(tile);
      }
      int num = Mathf.CeilToInt(VehicleCaravan_PathFollower.CostToMove(vehicleDefs, ticksPerMove,
      ...
      public VehicleCaravanInfo(List<Pawn> pawns)
      {
        ...
        massCapacity = pawns.Sum(pawn =>
          pawn is VehiclePawn vehicle ?
            ...
            (pawn.IsInVehicle() ? 0 : MassUtility.Capacity(pawn)));
      }
