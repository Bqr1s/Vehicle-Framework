...
namespace Vehicles
{
  public class AirDefensePositionTracker : WorldComponentTemp //Reimplement as world comp
  {
      ...
      if (VehicleMod.settings.main.airDefenses)
      {
        foreach (var defense in searchingDefenses)
        {
          ...
          for (int j = defense.Value.Count - 1; j >= 0; j--)
          {
            ...
            float distance = Ext_Math.SphericalDistance(airDefense.parent.DrawPos,
              aerialVehicleSearchingFor.DrawPos);
            bool withinMaxDistance = distance <= airDefense.MaxDistance;
            if (airDefense.CurrentTarget != aerialVehicleSearchingFor)
            {
              airDefense.angle = (airDefense.angle + RotationRate * airDefense.searchDirection)
               .ClampAngle();
              float angleToTarget =
                airDefense.parent.DrawPos.AngleToPoint(aerialVehicleSearchingFor.DrawPos);
              if (withinMaxDistance &&
                Mathf.Abs(angleToTarget - airDefense.angle) <= (airDefense.Arc / 2))
              {
            ...
            {
              float headingToTarget = WorldHelper.TryFindHeading(airDefense.parent.DrawPos,
                airDefense.CurrentTarget.DrawPos);
              int dirSignMultiplier = headingToTarget < airDefense.angle ? -2 : 2;
              if (Mathf.Abs(headingToTarget - airDefense.angle) < 1 ||
                Mathf.Abs(headingToTarget - airDefense.angle) > 359)
              {
              ...
              {
                airDefense.angle =
                  (airDefense.angle + RotationRate * dirSignMultiplier).ClampAngle();
              }
              if (!withinMaxDistance)
              {
    ...
    public static void RegisterAerialVehicle(AerialVehicleInFlight aerialVehicle,
      List<AirDefense> newDefenses)
    {
      ...
      foreach (AirDefense airDefense in newDefenses)
      {
        ...
        {
          airDefense.angle =
            ((Find.TickManager.TicksGame + airDefense.parent.GetHashCode()) * RotationRate) % 360;
        }
      }
    }
    ...
    public static List<AirDefense> GetNearbyObjects(AerialVehicleInFlight aerialVehicle,
    ...
    public override void ExposeData()
    {
      ...
      Scribe_Collections.Look(ref airDefenseCache, "airDefenseCache", LookMode.Reference,
        LookMode.Deep, ref saveableWorldObjects, ref saveableAirDefenses);
    }
  }
}