...
namespace Vehicles
{
  public class AirDefense : IExposable, ILoadReferenceable
  {
    ...
    public virtual AntiAircraftWorker Worker
    {
      get
      {
        worker ??= (AntiAircraftWorker)Activator.CreateInstance(antiAircraftDef.antiAircraftWorker,
          new object[] { this, antiAircraftDef });
        return worker;
      }
    }
    ...
    public Mesh SpotlightMesh
    {
      get
      {
        coneMesh ??= RenderHelper.NewConeMesh(Find.WorldGrid.AverageTileSize,
          antiAircraftDef.properties.arc);
        return coneMesh;
      }
    }
    ...
    public virtual void DrawSpotlightOverlay()
    {
      if (Worker.ShouldDrawSearchLight)
      {
        ...
        Quaternion quat = Quaternion.LookRotation(direction, normalized) *
          Quaternion.Euler(0f, angle - 90, 0f);
