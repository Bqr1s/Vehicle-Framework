...
public static class DynamicShadows
{
  ...
  public static Mesh GetShadowMesh(Texture2D texture, float baseWidth, float baseHeight,
    float tallness)
  {
  ...
  private static Mesh CreateMeshFromTexture(Texture2D texture, float baseWidth, float baseHeight,
    float tallness)
  {
          ...
          for (int i = 0; i < 2; i++)
          {
            for (int j = 0; j < 2; j++)
            {
              ...
              if (pixelColor.a >= AlphaIsoThreshold)
              {
                ...
                value |= 1 <<
                  contourIndex; //Sets 2nd to 5th bit, or 1 -> 2 -> 4 -> 8 CCW starting from bottom left corner
              }
            }
          }
  ...
  private static int HashOf(Texture2D texture, float baseWidth, float baseheight, float tallness)
  {
    ...
    return Gen.HashCombineInt(num * 391 ^ 261231 ^ num2 * 612331 ^ num3 * 456123,
      texture.GetHashCode());
  }
}