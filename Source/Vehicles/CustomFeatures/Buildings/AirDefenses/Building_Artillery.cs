...
namespace Vehicles
{
  ...
  public class Building_Artillery : Building_TurretGun
  {
    ...
    private static readonly Material AimPieMaterial =
      SolidColorMaterials.SimpleSolidColorMaterial(new Color(1f, 1f, 1f, 0.3f), false);
    ...
    {
      get { return burstWarmupTicksLeft > 0; }
    }

    public bool PlayerControlled
    {
      get { return (Faction == Faction.OfPlayer || MannedByColonist) && !MannedByNonColonist; }
    }
    ...
    {
      get { return def.building.IsMortar; }
    }

    public bool IsMortarOrProjectileFliesOverhead
    {
      get { return AttackVerb.ProjectileFliesOverhead() || IsMortar; }
    }
    ...
    public bool CanExtractShell
    {
      get
      {
        ...
        CompChangeableProjectile compChangeableProjectile =
          gun.TryGetComp<CompChangeableProjectile>();
    ...
    public bool MannedByColonist
    {
      get
      {
        return mannableComp != null && mannableComp.ManningPawn != null &&
    ...
    protected override void Tick()
    {
      ...
      if (Active && (mannableComp is null || mannableComp.MannedNow) && !IsStunned && Spawned &&
    ...
    {
      GenPlace.TryPlaceThing(gun.TryGetComp<CompChangeableProjectile>().RemoveShell(), Position,
        Map, ThingPlaceMode.Near, null, null, default);
    }
    ...
    protected new virtual void BeginBurst()
    {
      if (CurrentWorldTarget.IsValid)
      {
        ...
        {
          worldRecoiledVerb.TryStartCastOn(CurrentWorldTarget, TopArtillery.CurRotation, false,
            false);
        }
    ...
    protected new virtual void TryStartShootSomething(bool canBeginBurstImmediately)
    {
      ...
      if (!Spawned ||
        ((bool)AccessTools.Field(typeof(Building_TurretGun), "holdFire")?.GetValue(this) &&
          CanToggleHoldFire) ||
        (AttackVerb.ProjectileFliesOverhead() && Map.roofGrid.Roofed(Position)) ||
    ...
    public override void DrawExtraSelectionOverlays()
    {
      ...
      if (WarmingUp)
      {
        ...
        if (CurrentTarget.IsValid || CurrentWorldTarget.IsValid)
        {
          ...
          {
            Vector3 pieOffsetTmp =
              new Vector3(animatedProps.shootOffset.x, 0, animatedProps.shootOffset.y).RotatedBy(
                TopArtillery.CurRotation);
            pieOffset.x = pieOffsetTmp.x;
            pieOffset.y = pieOffsetTmp.z;
          }
          Vector3 root = DrawPos + new Vector3(pieOffset.x, offset, pieOffset.y);
          root += Quaternion.AngleAxis(TopArtillery.CurRotation, Vector3.up) * Vector3.forward *
            0.8f;
          Graphics.DrawMesh(MeshPool.pies[degreesWide], root,
            Quaternion.AngleAxis(TopArtillery.CurRotation + (degreesWide / 2) - 90f, Vector3.up),
            AimPieMaterial, 0);
        }
      }
