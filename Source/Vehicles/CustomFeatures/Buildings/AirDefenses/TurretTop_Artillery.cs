...
namespace Vehicles
{
  public class TurretTop_Artillery : TurretTop
  {
    ...
    public virtual new float CurRotation
    {
      get => curRotationInt;
      set => curRotationInt = value.ClampAngle();
    }
    ...
    public Graphic TurretGraphic
    {
      get
      {
        ...
        {
          turretGraphic = TurretGraphicData?.Graphic.GetColoredVersion(
            TurretGraphicData.shaderType.Shader, parentTurret.DrawColor, parentTurret.DrawColorTwo);
        }
    ...
    {
      Vector3 offset =
        new Vector3(parentTurret.def.building.turretTopOffset.x, 0f,
          parentTurret.def.building.turretTopOffset.y).RotatedBy(CurRotation);
      float drawSize = parentTurret.def.building.turretTopDrawSize;
      Matrix4x4 matrix = default;
      matrix.SetTRS(parentTurret.DrawPos + altitudeDrawLayer + Altitudes.AltIncVect,
        (CurRotation + ArtworkRotation).ToQuat(), new Vector3(drawSize, 1f, drawSize));
      Graphics.DrawMesh(MeshPool.plane10, matrix,
        TurretGraphic?.MatAt(parentTurret.Rotation) ?? parentTurret.def.building.turretTopMat, 0);
      if (DrawLayer != null)
      {
        DrawLayer.DrawExtra(
          parentTurret.DrawPos + altitudeDrawLayer + Altitudes.AltIncVect + offset, CurRotation);
      }
    }
    ...
    public virtual void Tick()
    {
      ...
      {
        float curRotation =
          (currentTarget.Cell.ToVector3Shifted() - parentTurret.DrawPos).AngleFlat();
      ...
      if (worldTarget.IsValid)
      {
        ...
        Vector3 target = worldTarget.WorldObject is null ?
          Find.WorldGrid.GetTileCenter(worldTarget.Tile) :
          worldTarget.WorldObject.DrawPos;
