...
namespace Vehicles
{
  ...
  public class Verb_ShootWorldRecoiled : Verb_ShootRecoiled
  {
    ...
    public virtual bool TryStartCastOn(GlobalTargetInfo castTarg, float heading,
      bool surpriseAttack = false, bool canHitNonTargetPawns = true)
    {
      if (caster is null)
      {
        Log.Error("Verb " + GetUniqueLoadID() +
          " needs caster to work (possibly lost during saving/loading).");
        return false;
      }
      if (!caster.Spawned)
      {
      ...
      IntVec3 exitTarget =
        caster.Position.PointFromAngle(Building_Artillery.MaxMapDistance, this.heading);
      ...
      {
        CasterPawn.Drawer.Notify_WarmingCastAlongLine(new ShootLine(caster.Position, exitTarget),
          caster.Position);
    ...
    {
      IntVec3 exitTarget =
        caster.Position.PointFromAngle(Building_Artillery.MaxMapDistance, heading);
      ...
      Projectile projectile2 =
        (Projectile)GenSpawn.Spawn(projectile, caster.Position, caster.Map, WipeMode.Vanish);
      if (projectile2.AllComps.NullOrEmpty())
      {
        AccessTools.Field(typeof(ThingWithComps), "comps")
         .SetValue(projectile2, new List<ThingComp>());
      }
      ...
      launchPos +=
        new Vector3(VerbProps.shootOffset.x, 0, VerbProps.shootOffset.y).RotatedBy(heading);
      projectile2.Launch(launcher, launchPos, exitTarget, exitTarget,
        ProjectileHitFlags.IntendedTarget, false, equipment);
      ThrowDebugText("Hit\nDest", shootLine.Dest);
