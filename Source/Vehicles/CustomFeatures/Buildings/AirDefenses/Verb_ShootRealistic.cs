  ...
  public class Verb_ShootRealistic : Verb_Shoot
  {
    ...
    protected virtual bool CausesTimeSlowdown(LocalTargetInfo target)
    {
      ...
      if (thing.def.category != ThingCategory.Pawn &&
        (thing.def.building == null || !thing.def.building.IsTurret))
      {
        return false;
      }
      ...
      return (thing.Faction == Faction.OfPlayer && caster.HostileTo(Faction.OfPlayer)) ||
        (caster.Faction == Faction.OfPlayer && thing.HostileTo(Faction.OfPlayer) && !flag);
    }
    ...
    protected virtual (bool success, Vector3 launchPos, float angle) TryCastShotInternal()
    {
      ...
      if (verbProps.ForcedMissRadius > 0.5f)
      {
        ...
        if (num > 0.5f)
        {
          ...
          if (num2 > 0)
          {
            ...
            launchPos +=
              new Vector3(VerbProps.shootOffset.x, 0, VerbProps.shootOffset.y).RotatedBy(angle);
            ...
            projectile2.Launch(launcher, launchPos, c, currentTarget, projectileHitFlags, false,
              equipment);
      ...
      if (!Rand.Chance(shotReport.AimOnTargetChance_IgnoringPosture))
      {
        shootLine.ChangeDestToMissWild(shotReport.AimOnTargetChance_StandardTarget,
          projectile.projectile.flyOverhead, caster.Map);
        ...
        launchPos +=
          new Vector3(VerbProps.shootOffset.x, 0, VerbProps.shootOffset.y).RotatedBy(angle);
        projectile2.Launch(launcher, launchPos, shootLine.Dest, currentTarget, projectileHitFlags2,
        ...
        !Rand.Chance(shotReport.PassCoverChance))
      {
        ...
        launchPos +=
          new Vector3(VerbProps.shootOffset.x, 0, VerbProps.shootOffset.y).RotatedBy(angle);
        projectile2.Launch(launcher, launchPos, randomCoverToMissInto, currentTarget,
          projectileHitFlags3, false, equipment, targetCoverDef);
        return (true, launchPos, angle);
      }
      ...
      if (currentTarget.Thing != null)
      {
        ...
        launchPos +=
          new Vector3(VerbProps.shootOffset.x, 0, VerbProps.shootOffset.y).RotatedBy(angle);
        projectile2.Launch(launcher, launchPos, currentTarget, currentTarget, projectileHitFlags4,
          false, equipment, targetCoverDef);
      ...
      else
      {
        ...
        launchPos +=
          new Vector3(VerbProps.shootOffset.x, 0, VerbProps.shootOffset.y).RotatedBy(angle);
        projectile2.Launch(launcher, launchPos, shootLine.Dest, currentTarget, projectileHitFlags4,
          false, equipment, targetCoverDef);
        ThrowDebugText("Hit\nDest", shootLine.Dest);
      }
      return (true, launchPos, angle);
        ...
        foreach (AnimationProperties moteProps in VerbProps.motes)
        {
          ...
          if (loc.ShouldSpawnMotesAt(caster.Map))
          {
            try
            {
              ...
              moteLoc += new Vector3(VerbProps.shootOffset.x + moteProps.offset.x,
                  altitudeLayer + moteProps.offset.y, VerbProps.shootOffset.y + moteProps.offset.z)
               .RotatedBy(angle);
