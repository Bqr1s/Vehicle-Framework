...
namespace Vehicles
{
  public class TurretUpgrade : Upgrade
  {
    ...
    public override bool HasGraphics => turrets.NotNullAndAny(turret => !turret.NoGraphic) ||
      !removeTurrets.NullOrEmpty();
      ...
      if (!unlockingPostLoad)
      {
        ...
        if (!turrets.NullOrEmpty())
        {
          foreach (VehicleTurret turret in turrets)
          {
            ...
            {
              Log.Error(
                $"{VehicleHarmony.LogLabel} Unable to unlock {GetType()} to {vehicle.LabelShort}. \nException: {ex}");
            }
          }
        }
      }
      ...
      if (!removeTurrets.NullOrEmpty())
      {
        foreach (string key in removeTurrets)
        {
          VehicleTurret turret =
            vehicle.CompVehicleTurrets.Props.turrets.FirstOrDefault(turret => turret.key == key);
          if (turret == null)
          {
            Log.Error(
              $"Unable to add {key} to {vehicle}. Turret must be defined in the VehicleDef in order to be re-added post-refund.");
          }
          else
          {
