  ...
  public abstract class VehicleSkyfaller_FlyOver : VehicleSkyfaller
  {
      ...
      get
      {
        switch (def.skyfaller.movementType)
        {
          case SkyfallerMovementType.Accelerate:
            return SkyfallerHelper.DrawPos_Accelerate(base.DrawPos,
              vehicle.CompVehicleLauncher.launchProtocol.TicksPassed, angle, CurrentSpeed);
          case SkyfallerMovementType.ConstantSpeed:
            return SkyfallerHelper.DrawPos_ConstantSpeed(base.DrawPos,
              vehicle.CompVehicleLauncher.launchProtocol.TicksPassed, angle, CurrentSpeed);
          case SkyfallerMovementType.Decelerate:
            return SkyfallerHelper.DrawPos_Decelerate(base.DrawPos,
              vehicle.CompVehicleLauncher.launchProtocol.TicksPassed, angle, CurrentSpeed);
          default:
    ...
    protected virtual float TimeInAnimation
    {
      get
      {
        ...
        {
          return (float)vehicle.CompVehicleLauncher.launchProtocol.TicksPassed /
            def.skyfaller.ticksToImpactRange.max;
        }
        return 1f - (float)vehicle.CompVehicleLauncher.launchProtocol.TicksPassed /
          def.skyfaller.ticksToImpactRange.max;
      }
    }
    ...
    public Vector3 DistanceAtMin
    {
      ...
      {
        return SkyfallerHelper.DrawPos_ConstantSpeed(base.DrawPos,
          -def.skyfaller.ticksToImpactRange.min, angle, CurrentSpeed);
      }
    }
    ...
    public override void SpawnSetup(Map map, bool respawningAfterLoad)
    {
      ...
      if (!respawningAfterLoad)
      {
        ...
        vehicle.CompVehicleLauncher.launchProtocol.SetTickCount(-def.skyfaller.ticksToImpactRange
         .max);
      }
    }
  }
