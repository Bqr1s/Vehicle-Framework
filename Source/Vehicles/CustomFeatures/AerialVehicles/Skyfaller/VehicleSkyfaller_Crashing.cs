  ...
  public class VehicleSkyfaller_Crashing : VehicleSkyfaller_Arriving
  {
      ...
      get
      {
        switch (def.skyfaller.movementType)
        {
          case SkyfallerMovementType.Accelerate:
            return SkyfallerDrawPosUtility.DrawPos_Accelerate(base.DrawPos, ticksToImpact, angle,
              CurrentSpeed);
          case SkyfallerMovementType.ConstantSpeed:
            return SkyfallerDrawPosUtility.DrawPos_ConstantSpeed(base.DrawPos, ticksToImpact, angle,
              CurrentSpeed);
          case SkyfallerMovementType.Decelerate:
            return SkyfallerDrawPosUtility.DrawPos_Decelerate(base.DrawPos, ticksToImpact, angle,
              CurrentSpeed);
          default:
    ...
    protected virtual float CurrentSpeed
    {
      get
      {
        ...
        return def.skyfaller.speedCurve.Evaluate(vehicle.CompVehicleLauncher.launchProtocol
         .TimeInAnimation) * def.skyfaller.speed;
      }
    }
    ...
    protected override void DrawAt(Vector3 drawLoc, bool flip = false)
    {
      ...
      {
        angle = def.skyfaller.angleCurve.Evaluate(vehicle.CompVehicleLauncher.launchProtocol
         .TimeInAnimation);
      }
      if (def.skyfaller.rotationCurve != null)
      {
        num += def.skyfaller.rotationCurve.Evaluate(vehicle.CompVehicleLauncher.launchProtocol
         .TimeInAnimation);
      }
      if (def.skyfaller.xPositionCurve != null)
      {
        drawLoc.x +=
          def.skyfaller.xPositionCurve.Evaluate(vehicle.CompVehicleLauncher.launchProtocol
           .TimeInAnimation);
      }
      if (def.skyfaller.zPositionCurve != null)
      {
        drawLoc.z +=
          def.skyfaller.zPositionCurve.Evaluate(vehicle.CompVehicleLauncher.launchProtocol
           .TimeInAnimation);
      }
    ...
    protected virtual void Impact()
    {
      ...
      {
        GenExplosion.DoExplosion(Position, Map, def.skyfaller.explosionRadius,
          def.skyfaller.explosionDamage, null,
          GenMath.RoundRandom(def.skyfaller.explosionDamage.defaultDamage *
            def.skyfaller.explosionDamageFactor),
          ignoredThings: (!def.skyfaller.damageSpawnedThings) ?
            vehicle.inventory.innerContainer.ToList() :
            null);
      }
      //this.SpawnThings();
      CellRect cellRect = this.OccupiedRect();
      ...
      {
        def.skyfaller.impactSound.PlayOneShot(SoundInfo.InMap(new TargetInfo(Position, Map, false),
