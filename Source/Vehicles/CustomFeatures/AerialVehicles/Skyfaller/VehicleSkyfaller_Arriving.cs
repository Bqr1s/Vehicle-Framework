  ...
  public class VehicleSkyfaller_Arriving : VehicleSkyfaller
  {
    ...
    public CompProperties_VehicleLauncher VehicleLauncherProps
    {
      get
      {
        ...
        {
          vehicleLauncherProps =
            vehicle.VehicleDef.GetCompProperties<CompProperties_VehicleLauncher>();
        }
    ...
    protected override void Tick()
    {
      ...
      else if (!punchedRoof && vehicle.CompVehicleLauncher.launchProtocol.TimeInAnimation >=
        VehicleLauncherProps.animationPunchAt)
      {
        TryHitRoof();
      }
    ...
    protected virtual void FinalizeLanding()
    {
      ...
      if (VehicleReservationManager.AnyVehicleInhabitingCells(
        vehicle.PawnOccupiedCells(Position, LandingRotation), Map))
      {
        GenExplosion.DoExplosion(Position, Map,
          Mathf.Max(vehicle.VehicleDef.Size.x, vehicle.VehicleDef.Size.z), DamageDefOf.Bomb,
          vehicle);
      }
      else
      {
    ...
    private void DrawLandingGhost()
    {
      ...
      {
        Color ghostcolor = LandingSpotOccupied ?
          LandingTargeter.GhostOccupiedColor :
          VehicleGhostUtility.whiteGhostColor;
        GhostDrawer.DrawGhostThing(Position, LandingRotation, vehicle.VehicleDef,
          vehicle.VehicleGraphic, ghostcolor, AltitudeLayer.Blueprint, vehicle);
      }
    }
    ...
    protected virtual void TryHitRoof()
    {
      ...
      {
        RoofDef roofDef = cellRect.Cells.First(cell => Ext_Vehicles.IsRoofed(cell, Map))
         .GetRoof(Map);
        if (!roofDef.soundPunchThrough.NullOrUndefined())
        {
          roofDef.soundPunchThrough.PlayOneShot(new TargetInfo(Position, Map, false));
        }
        RoofCollapserImmediate.DropRoofInCells(cellRect.ExpandedBy(1).ClipInsideMap(Map).Cells
         .Where(delegate(IntVec3 c)
          {
            if (!c.InBounds(Map))
            {
              return false;
