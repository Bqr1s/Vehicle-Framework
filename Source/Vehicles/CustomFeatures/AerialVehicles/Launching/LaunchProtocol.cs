...
namespace Vehicles
{
  ...
  public abstract class LaunchProtocol : IExposable
  {
      ...
      get
      {
          ...
          action = delegate()
          {
            if (vehicle.CompVehicleLauncher.AnyLeftToLoad)
            {
              Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(
                "ConfirmSendNotCompletelyLoadedPods".Translate(vehicle.LabelCapNoCount),
        ...
        if (vehicle.Spawned && LaunchProperties.restriction != null)
        {
          takeoffCommand.mouseOver = () =>
            LaunchProperties.restriction.DrawRestrictionsTargeter(vehicle, vehicle.Map,
    ...
    {
      if (AerialVehicleArrivalAction_FormVehicleCaravan.CanFormCaravanAt(vehicle, tile) &&
      ...
      else if (Find.WorldObjects.MapParentAt(tile) is MapParent mapParent)
      {
        ...
        {
          foreach (FloatMenuOption option in AerialVehicleArrivalAction_LoadMap.GetFloatMenuOptions(
            vehicle, this, mapParent))
          {
            yield return option;
          }
        }
        ...
        if (vehicle.CompVehicleLauncher.ControlInFlight &&
          vehicle.CompVehicleTurrets != null) //REDO - strafe specific properties
        {
      ...
      if (Find.WorldObjects.SettlementAt(tile) is Settlement settlement)
      {
        ...
        foreach (FloatMenuOption option in AerialVehicleArrivalAction_AttackSettlement
         .GetFloatMenuOptions(vehicle, this, settlement))
        {
          yield return option;
        }
      }
      ...
      return new FloatMenuOption("FormCaravanHere".Translate(), delegate()
      {
        if (vehicle.Spawned)
        {
          vehicle.CompVehicleLauncher.TryLaunch(tile,
            new AerialVehicleArrivalAction_FormVehicleCaravan(vehicle));
        }
        else
        {
          AerialVehicleInFlight aerialVehicle =
            AerialVehicleLaunchHelper.GetOrMakeAerialVehicle(vehicle);
          aerialVehicle.OrderFlyToTiles(LaunchTargeter.FlightPath, aerialVehicle.DrawPos,
            new AerialVehicleArrivalAction_FormVehicleCaravan(vehicle));
        }
      ...
      return new FloatMenuOption("LandInExistingMap".Translate(vehicle.Label), delegate()
      {
        ...
        LandingTargeter.Instance.BeginTargeting(vehicle,
          action: delegate(LocalTargetInfo target, Rot4 rot)
          {
            ...
            {
              vehicle.CompVehicleLauncher.TryLaunch(tile,
                new AerialVehicleArrivalAction_LandSpecificCell(vehicle, mapParent, tile,
                  target.Cell, rot));
            }
            else
            {
    ...
    protected virtual FloatMenuOption FloatMenuOption_ReconMap(MapParent mapParent, int tile)
    {
      if (mapParent.EnterCooldownBlocksEntering())
      {
        return new FloatMenuOption(
          $"{"VF_AerialReconSite".Translate(mapParent.Label)} ({"EnterCooldownBlocksEntering".Translate()})",
          null);
      }
    ...
    protected virtual FloatMenuOption FloatMenuOption_StrafeMap(MapParent mapParent, int tile)
    {
      return new FloatMenuOption("VF_StrafeRun".Translate(), delegate()
      {
        ...
        CameraJumper.TryShowWorld();
        LaunchTargeter.Instance.RegisterActionOnTile(tile,
          new AerialVehicleArrivalAction_StrafeMap(vehicle, mapParent));
      }, MenuOptionPriority.Default, null, null, 0f, null, null);
    }

    protected virtual IEnumerable<FloatMenuOption> FloatMenuOption_LandAtSettlement(
      Settlement settlement, int tile)
    {
      ...
      if (AerialVehicleArrivalAction_Trade.CanTradeWith(vehicle, settlement))
      {
        yield return new FloatMenuOption("TradeWith".Translate(settlement.Label), delegate()
        {
          ...
          {
            vehicle.CompVehicleLauncher.TryLaunch(tile,
              new AerialVehicleArrivalAction_Trade(vehicle, settlement));
          }
          else
          {
            AerialVehicleInFlight aerialVehicle =
      ...
      if (AerialVehicleArrivalAction_OfferGifts.CanOfferGiftsTo(vehicle, settlement))
      {
        yield return new FloatMenuOption("OfferGifts".Translate(settlement.Label), delegate()
        {
          ...
          {
            vehicle.CompVehicleLauncher.TryLaunch(tile,
              new AerialVehicleArrivalAction_OfferGifts(vehicle, settlement));
          }
          else
          {
            AerialVehicleInFlight aerialVehicle =
    ...
    public virtual string TargetingLabelGetter(GlobalTargetInfo target, int tile,
      List<FlightNode> path, float fuelCost)
    {
      if (!target.IsValid)
      {
        return null;
      ...
      else if (vehicle.CompFueledTravel != null && target.IsValid &&
        vehicle.CompVehicleLauncher.FuelNeededToLaunchAtDist(WorldHelper.GetTilePos(tile),
    ...
    private bool ChoseWorldTarget(GlobalTargetInfo target, float fuelCost)
    {
        ...
        Action<int, AerialVehicleArrivalAction, bool> launchAction)
      {
        if (!target.IsValid)
        {
          ...
          vehicle.CompVehicleLauncher.MaxLaunchDistance || (vehicle.CompFueledTravel != null &&
    ...
    public static bool CanLandInSpecificCell(MapParent mapParent)
    {
      return mapParent != null && mapParent.Spawned && mapParent.HasMap &&
        (!mapParent.EnterCooldownBlocksEntering() ||
          FloatMenuAcceptanceReport.WithFailMessage("MessageEnterCooldownBlocksEntering".Translate(
            mapParent.EnterCooldownTicksLeft().ToStringTicksToPeriod(true, false, true, true))));
    }
