...
namespace Vehicles
{
  public class LaunchTargeter : BaseVehicleWorldTargeter
  {
    ...
    public static void BeginTargeting(VehiclePawn vehicle,
      Func<GlobalTargetInfo, float, bool> action, AerialVehicleInFlight aerialVehicle,
      bool canTargetTiles, Texture2D mouseAttachment = null, bool closeWorldTabWhenFinished = false,
      Action onUpdate = null,
      Func<GlobalTargetInfo, List<FlightNode>, float, string> extraLabelGetter = null)
    {
      Instance.vehicle = vehicle;
    ...
    public void ContinueTargeting(VehiclePawn vehicle, Func<GlobalTargetInfo, float, bool> action,
      AerialVehicleInFlight aerialVehicle, bool canTargetTiles, Texture2D mouseAttachment = null,
      bool closeWorldTabWhenFinished = false, Action onUpdate = null,
      Func<GlobalTargetInfo, List<FlightNode>, float, string> extraLabelGetter = null)
    {
      Instance.vehicle = vehicle;
      Instance.action = action;
    ...
    public override void ProcessInputEvents()
    {
      if (Event.current.type == EventType.MouseDown)
      {
        if (Event.current.button == 0 && IsTargeting)
        {
          ...
          if (WorldHelper.WorldObjectAt(arg.Tile) is WorldObject &&
            vehicle.CompVehicleLauncher.SpaceFlight && !maxNodesHit && !sameTile)
          {
          ...
          {
            Messages.Message("VF_FlightPathMaxNodes".Translate(vehicle.LabelShortCap),
      ...
      if ((Event.current.type == EventType.MouseDown && Event.current.button == 1) ||
        (KeyBindingDefOf.Cancel.KeyDownEvent && IsTargeting))
      {
        SoundDefOf.CancelMode.PlayOneShotOnCamera(null);
        StopTargeting();
        Event.current.Use();
    ...
    public override void TargeterOnGUI()
    {
      ...
      if (mouseTarget.IsValid &&
        FlightPath.Count < vehicle.CompVehicleLauncher.launchProtocol.MaxFlightNodes)
      {
        (fuelOnPathCost, sphericalDistance) = CostAndDistanceCalculator(flightPathOrigin,
          WorldHelper.GetTilePos(mouseTarget.Tile));
      ...
      Rect labelPosition = new Rect(mousePosition.x, mousePosition.y + textSize.y + 20f, textSize.x,
        textSize.y);
      ...
      if (extraLabelGetter != null)
      {
        ...
        Rect bgRect = new Rect(rect.x - labelGetterText.x * 0.1f, rect.y, labelGetterText.x * 1.2f,
          labelGetterText.y);
      ...
      {
        GUI.DrawTexture(
          new Rect(labelPosition.x - textSize.x * 0.1f, labelPosition.y, bgWidth, textSize.y),
          TexUI.GrayTextBG);
    ...
    public virtual void DrawAirDefenseGrid()
    {
      foreach (AirDefense airDefense in AirDefensePositionTracker.airDefenseCache.Values)
      {
        ...
        {
          RenderHelper.DrawWorldRadiusRing(airDefense.parent.Tile,
            Mathf.CeilToInt(airDefense.MaxDistance), TexData.OneSidedWorldLineMatRed);
        }
      }
    }
    ...
    public override void TargeterUpdate()
    {
      ...
      if (FlightPath.Count > 0)
      {
        ...
        WorldRendererUtility.DrawQuadTangentialToPlanet(start,
          BaseFeedbackTexSize * Find.WorldGrid.AverageTileSize, 0.018f,
          WorldMaterials.CurTargetingMat);
      }
      if (FlightPath.Count < vehicle.CompVehicleLauncher.launchProtocol.MaxFlightNodes &&
        arg.IsValid)
      {
    ...
    public bool IsTargetedNow(WorldObject worldObject,
