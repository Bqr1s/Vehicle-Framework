...
namespace Vehicles
{
  public static class VehicleTabHelper_Passenger
  {
      ...
      VehiclePawn vehicle, ref Pawn moreDetailsForPawn)
    {
      ...
      {
        VehicleRoleHandler handler = vehicle.handlers[i];
      ...
      IThingHolder holder, string label, List<Pawn> pawns, ref Pawn moreDetailsForPawn)
    {
      ...
      Rect handlerRect = new Rect(0, curY, viewRect.width - PawnExtraButtonSize * 2,
    ...
    private static bool DoRow(float curY, Rect viewRect, Vector2 scrollPos, Pawn pawn,
      ref Pawn moreDetailsForPawn, bool highlight)
    {
      ...
      Widgets.BeginGroup(pawnRect);
      {
        ...
        Rect iconRect = new Rect(PawnRowPadding, (pawnRect.height - ThingIconSize) / 2f,
        ...
        foreach (Need need in tmpNeeds)
        {
          ...
          need.DrawOnGUI(needRect, maxThresholdMarkers: maxThresholdMarkers, customMargin: 10,
            drawArrows: false, doTooltip: doTooltip);
          xMax = needRect.xMax;
        }
    ...
    private static void OpenSpecificTabButton(Rect rowRect, Pawn pawn, ref Pawn moreDetailsForPawn)
    {
      ...
      Rect rect = new Rect(rowRect.width - PawnExtraButtonSize,
    ...
    public static void HandleDragEvent()
    {
      if (Event.current.type == EventType.MouseUp && Event.current.button == 0)
      {
        ...
        {
          if (transferToHolder is VehicleRoleHandler transferToHandler &&
            !transferToHandler.AreSlotsAvailable)
          {
            ...
            {
              Messages.Message(
                TranslatorFormattedStringExtensions.Translate("VF_HandlerNotEnoughRoom",
                  draggedPawn, transferToHandler.role.label), MessageTypeDefOf.RejectInput);
            }
          }
          else if (draggedPawn.ParentHolder != transferToHolder)
          {
            ...
            {
              SoundDefOf.Click.PlayOneShotOnCamera();
              if (transferToHolder is VehicleRoleHandler validHandler)
              {
                if (previousHolder is VehicleRoleHandler)
                {
                  validHandler.vehicle.EventRegistry[VehicleEventDefOf.PawnChangedSeats]
                   .ExecuteEvents();
                }
                else
                {
