...
namespace Vehicles
{
  ...
  public static class HealthHelper
  {
    ...
    public static (int current, int max, string explanation) GetTotalHealth(
      this VehiclePawn vehicle)
    {
      ...
      foreach (VehicleComponent component in vehicle.statHandler.components)
      {
        ...
        explanation.AppendLine(
          $"    {StatDefOf.ArmorRating_Blunt.LabelCap}: {armorBlunt.ToStringPercent()}");
        float armorSharp = component.ArmorRating(DamageArmorCategoryDefOf.Sharp, out _);
        explanation.AppendLine(
          $"    {StatDefOf.ArmorRating_Sharp.LabelCap}: {armorSharp.ToStringPercent()}");
        float armorHeat = component.ArmorRating(heatArmor, out _);
        explanation.AppendLine(
          $"    {StatDefOf.ArmorRating_Heat.LabelCap}: {armorHeat.ToStringPercent()}");
        explanation.AppendLine();
      }
      ...
      this VehicleDef vehicleDef)
    {
      ...
      if (result.health == 0)
      {
        ...
        foreach (VehicleComponentProperties componentProperties in vehicleDef.components)
        {
          ...
          float armorBlunt =
            componentProperties.armor.GetStatValueFromList(StatDefOf.ArmorRating_Blunt,
              vehicleArmorBlunt);
          explanation.AppendLine(
            $"    {StatDefOf.ArmorRating_Blunt.LabelCap}: {armorBlunt.ToStringPercent()}");
          float armorSharp =
            componentProperties.armor.GetStatValueFromList(StatDefOf.ArmorRating_Sharp,
              vehicleArmorSharp);
          explanation.AppendLine(
            $"    {StatDefOf.ArmorRating_Sharp.LabelCap}: {armorSharp.ToStringPercent()}");
          float armorHeat =
            componentProperties.armor.GetStatValueFromList(StatDefOf.ArmorRating_Heat,
              vehicleArmorHeat);
          explanation.AppendLine(
            $"    {StatDefOf.ArmorRating_Heat.LabelCap}: {armorHeat.ToStringPercent()}");
          explanation.AppendLine();
